...
void get_header(unsigned char* packet, header_s &head)
{
    ...
    long long *word0 = reinterpret_cast<long long*>(packet+8);

    long long hword0 = ntohll(*word0);
    long long hword1 = ntohll(*word1);
    char *hword0c = reinterpret_cast<char*>(&hword0);
    char *hword1c = reinterpret_cast<char*>(&hword1);

    head.frame_no = (int)(hword0c[0] | (hword0c[1] << 8) | (hword0c[2] << 16) | (hword0c[3] << 24));
    head.ref_s = (int)(hword0c[4] | (hword0c[5] << 8) | (hword0c[6] << 16) | ((hword0c[7] << 24) & 0x3f000000));
    head.comp = (bool)((hword0c[7] & 0x40) >> 6);
    head.invalid = (bool)((hword0c[7] & 0x80) >> 7);

    head.in_bits = (int)(hword1c[15] & 0x1f);         // 0x1f -> 00011111
    head.version = (int)((hword1c[15] & 0xe0) >> 5);         // 0xe0 -> 11100000
...
//    head.station = (int)(packet[12] | (packet[13] << 16));
    ...
    cout << "Synchronisation word: " << std::hex << head.synch << endl;
